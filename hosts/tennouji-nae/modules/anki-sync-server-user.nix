# Modified from: https://github.com/NixOS/nixpkgs/blob/252726f4b7b33bcc2ef262629aaf84cc96cd5ab0/nixos/modules/services/misc/anki-sync-server.nix
...
with lib;
let
  cfg = config.services.anki-sync-server-user;
...
in
{
  options.services.anki-sync-server-user = {
    enable = mkEnableOption "anki-sync-server-user";
    ...
    user = mkOption {
      type = types.str;
      default = "anki-sync";
      description = "User to run the anki-sync-server service as.";
    };

    group = mkOption {
      type = types.str;
      default = "anki-sync";
      description = "Group to run the anki-sync-server service as.";
    };
