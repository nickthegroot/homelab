# Modified from: https://github.com/NixOS/nixpkgs/blob/252726f4b7b33bcc2ef262629aaf84cc96cd5ab0/nixos/modules/services/misc/anki-sync-user.nix
# Added `user` and `group` params to allow for more flexibility in baseDirectory placement
{
  config,
  lib,
  pkgs,
...
with lib;
let
  cfg = config.services.anki-sync-user;
  name = "anki-sync-user";
  ...
  anki-sync-user-run = pkgs.writeShellScript "anki-sync-user-run" ''
    # When services.anki-sync-user.users.passwordFile is set,
    ...
    # For users where services.anki-sync-user.users.password isn't set,
...
in
{
  options.services.anki-sync-user = {
    enable = mkEnableOption "anki-sync-user";
    ...
    address = mkOption {
      ...
      default = "::1";
      description = ''
        IP address anki-sync-user listens to.
    ...
    port = mkOption {
      type = types.port;
      default = 27701;
      description = "Port number anki-sync-user listens to.";
    };
    ...
    user = mkOption {
      type = types.str;
      default = "anki-sync";
      description = "User to run the anki-sync-user service as.";
    };

    group = mkOption {
      type = types.str;
      default = "anki-sync";
      description = "Group to run the anki-sync-user service as.";
    };
      ...
      type =
        ...
        listOf (submodule {
          options = {
            username = mkOption {
              type = str;
              description = "User name accepted by anki-sync-user.";
            };
            password = mkOption {
              type = nullOr str;
              default = null;
              description = ''
                Password accepted by anki-sync-user for the associated username.
                **WARNING**: This option is **not secure**. This password will
                be stored in *plaintext* and will be visible to *all users*.
                See {option}`services.anki-sync-user.users.passwordFile` for
            ...
            passwordFile = mkOption {
              ...
              default = null;
              description = ''
                File containing the password accepted by anki-sync-user for
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.anki-sync-user = {
      description = "anki-sync-user: Anki sync server built into Anki";
